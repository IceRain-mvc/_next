...
const Details = props => {
  ...
  const {
    0: dual,
    1: setDual
  } = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)(true); // 控制开关

  const {
    0: flag,
    1: setFlag
  } = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)(false); // 控制显示隐藏
  ...
  const {
    0: dataCount,
    1: setDataCount
  } = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)(0); // 分页总条数

  const {
    0: page,
    1: setPage
  } = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)(1); // 筛选条件  联动查询
  ...
  const {
    0: condition,
    1: setCondition
  } = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)('考试状态'); // 考试状态

  const {
    0: passState,
    1: setPassState
  } = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)('是否及格'); // 是否及格

  const {
    0: today,
    1: setToday
  } = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)(undefined); // 日期筛选 今天 昨天 近七天

  const {
    0: time,
    1: setTime
  } = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)(undefined); // 自定义日期

  const {
    0: keyword,
    1: setKeyword
  } = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)(undefined); // 关键字查询
  ...
  const {
    0: total,
    1: setTotal
  } = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)(); // 鼠标划入判断id

  const voluntarily = (0,react__WEBPACK_IMPORTED_MODULE_2__.useRef)(null); // 获取input框数据

  const {
    0: simpleness,
    1: setSimpleness
  } = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)(); // onmouseover划入获取id
      ...
      if (res) {
        ...
        if (res.code === '3') {
          if (res.data[0].length > 0) {
            antd_lib_message__WEBPACK_IMPORTED_MODULE_6___default().success('查询成功', 1);
          } else {
            antd_lib_message__WEBPACK_IMPORTED_MODULE_6___default().warning('没有数据', 1);
          }
        }
      }
    });
  ...
  const Research = (0,react__WEBPACK_IMPORTED_MODULE_2__.useRef)(null); // 分页的点击事件
  ...
  const modifications = async item => {
    ...
    await (0,_api_analyse_alalyse_statistical__WEBPACK_IMPORTED_MODULE_4__/* .getUpdate */ .YX)({
      ...
      if (res.code === '01') {
        antd_lib_message__WEBPACK_IMPORTED_MODULE_6___default().success('修改成功', 0.5);
      } else if (res.code === '02') {
        antd_lib_message__WEBPACK_IMPORTED_MODULE_6___default().error('成绩不能为空', 0.5);
      }
    });
    getDate({
      keyword,
        ...
        children: /*#__PURE__*/react_jsx_runtime__WEBPACK_IMPORTED_MODULE_7__.jsx(_component_form_form__WEBPACK_IMPORTED_MODULE_3__.ButtonCommon, {
          onClick: () => {
            if (Research.current.state.value || time || condition !== '考试状态' || passState !== '是否及格' || today) {
              ...
              antd_lib_message__WEBPACK_IMPORTED_MODULE_6___default().error('请至少输入一个参数');
            }
          },
          type: 'none',
          children: "\u67E5\u8BE2"
